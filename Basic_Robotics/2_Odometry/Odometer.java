...
public class Odometer extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			rightDistance = Lab2.WHEEL_HALF_CIRC * tachoDistance / 180;

			tachoDistance = newLeftTacho - oldLeftTacho;
			leftDistance = Lab2.WHEEL_HALF_CIRC * tachoDistance / 180;
			
			thetaDistance = (leftDistance - rightDistance) / Lab2.TRACK;
			...
			synchronized (lock) {
				...
				theta = (theta + thetaDistance) % Lab2.RAD_CIRC;
				// avoid negatives
				if(theta < 0) theta += Lab2.RAD_CIRC;
				
				x += posDistance*Math.sin(theta);
				y += posDistance*Math.cos(theta);
			}
