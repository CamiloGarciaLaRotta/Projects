...
public class USLocalizer {
	...
	public void doLocalization() {
		...
		if (locType == LocalizationType.FALLING_EDGE) {
			...
			leftMotor.rotate(
					convertAngle(Lab4.WHEEL_RADIUS, Lab4.TRACK, zeroTheta),
					true);
			rightMotor.rotate(
					-convertAngle(Lab4.WHEEL_RADIUS, Lab4.TRACK, zeroTheta),
					false);

			// update the odometer position to 0 0 0 Position (x and y) will be
			// wrong but that will be fixed by the LightLocalizer
			odo.setPosition(new double[] { 0.0, 0.0, 0 }, new boolean[] { true,
					true, true });
		} else {
			// rotate until a wall
			status = "efirst";
